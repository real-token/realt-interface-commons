...
export function Layout<T extends Partial<Chain>>({
  ...
  useEffect(() => {
    if (!chains) return;
    if (chains.allowedChains.length !== Object.keys(chains.chainsConfig).length)
      throw new Error(
        "Allowed chains and chains config must have the same length"
      );
  }, [chains]);
  ...
  return (
    <div className={classes.container}>
      ...
      <Header
        ...
        disabledWrongNetworkBanner={disabledWrongNetworkBanner}
      />
